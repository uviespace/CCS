...
def run(jfile, outfile, reportfunc=False, specfile=None, mib=None):
    ...
    if reportfunc:
        ...
        if specfile is None:
            # specfile = '{}-TS-{}.jinp'.format(data['_name'], data['_spec_version'])
            specfile = outfile.replace('.py', '.jinp')

        # save JSON reference input file along with py script
        jout = os.path.join(os.path.dirname(outfile), os.path.basename(specfile))
        with open(jout, 'w') as fd:
            json.dump(data, fd, indent=2)

        script += 'from reporting import TestReport\n\n'.format(specfile)
        script += 'specfile = "{}"\n'.format(specfile)
        script += 'rep_version = 1\n'
        script += 'mib_version = "{}"\n'.format(mib)
        script += 'ask_tc_exec = True\n'
        script += 'report = TestReport(specfile, rep_version, mib_version, gui=True)\n\n'
    ...
    for step in data['sequences'][0]['steps']:
        ...
        if reportfunc:
            step_tag = '{}'.format(step['_step_number'])
        ...
        txt = '# {}_{}\n' \
              ...
              '{}' \
              '{}' \
              '# VERIFICATION: {}\n{}{}\n#! CCS.BREAKPOINT\n\n'.format(data['_name'], step['_step_number'], replace_newline(step['_description']), exec_step,
                                                                      cmd_code,
