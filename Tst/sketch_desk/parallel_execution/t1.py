...
class ExampleOne:
    ...
    def step_1(self, ccs, pool_name, queue, configurer, event, *args):
        ...
        try:
            ...
            i = 0
            while i < 5:
                logger.info('testscript_1: step_1: count = {}'.format(i))
                time.sleep(0.5)
                if i is 3:
                    event.set()
                i += 1
    ...
    # STEP 2 --------------------------------------------------------------------------------------------------------
    def step_2(self, ccs, pool_name, queue, configurer, event, *args):
        configurer(queue)
        logger = logging.getLogger(__name__)
        param = {
            'step_no': '2',
        ...
        try:
            i = 0
            while i < 5:
                logger.info('testscript_1: step_2: count = {}'.format(i))
                time.sleep(0.5)
                i += 1
            # # send TC(3,5)
    ...
    # STEP 3 --------------------------------------------------------------------------------------------------------
    def step_3(self, ccs, pool_name, queue, configurer, event, *args):
        configurer(queue)
        logger = logging.getLogger(__name__)
        param = {
            'step_no': '3',
        ...
        try:
            i = 0
            while i < 5:
                logger.info('testscript_1: step_3: count = {}'.format(i))
                time.sleep(0.5)
                i += 1
        except Exception as e:
            report.command_step_exception(step_param=param)
    ...
    def run(self, ccs, pool_name, queue, configurer, save_pool=True, loglevel='DEBUG', make_new_log_file=True):
        """
