...
mem_addr = 0x40400000  # address where the data is uploaded to
...
cfl.upload_srec_segmented(srecfile, memid, mem_addr, segid, pool_name=POOLNAME, max_pkt_size=504, progress=True)  # optionally, provide the name of the TC that shall be used for upload, e.g., tcname='DBS_TC_LOAD_MEMORY'

#! CCS.BREAKPOINT
CH_MEM_MID = "DPU_MRAM"  # KTP40060
CH_START_ADDR = mem_addr  # KTP40061
CH_BLOCK_LEN = 431800  # KTP40062
...
dlen, dcrc = cfl.srec_direct(srecfile, memid, pool_name=POOLNAME, max_pkt_size=200, tcname=tc_cmd, sleep=0.125, byte_align=2)
...
thread = threading.Thread(target=cfl.upload_srec, args=[srecfile, memid, mem_addr, segid], kwargs={'pool_name':POOLNAME,'max_pkt_size':504, 'progress':True})
thread.start()
