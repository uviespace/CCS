...
It is assumed that the software has already been successfully installed and set up as described at \url{https://github.com/uviespace/CCS/tree/release}. 
Furthermore, this manual assumes that an application is running on the target machine and accessible via port XXXX. 
Finally, it is also assumed that a Mission Information Base (MIB) compatible with the application has already been imported.
...
\section{Quick Start}
In the following section, a concise set of instructions is described for launching \textit{ccs} and establishing a basic connection. 
Only the essential steps required to start \textit{ccs}, connect it via the designated port, and verify the initial interfaces are running are outlined below. 
Upon startup, several windows appear. These are briefly introduced, while detailed explanations are provided in later chapters.

\begin{itemize}

\item Navigate to the \texttt{CCS} directory. \footnote{Within \texttt{CCS} there is also a directory called \texttt{Ccs}. Therefore "\textit{ccs}" will mean the software itself, "\texttt{CCS}" and "\texttt{Ccs}" the respective directories} From here the tst and \textit{ccs} tools can be started. Seeing as one can still be started within the menu of the other, opening only one is sufficient for now.

\item Run \texttt{./start\_ccs} to start the software.

\item The \textit{ccs} Editor should open. Before running the \texttt{connection\_setup.py} script a few things have to be configured.

\begin{itemize}
  \renewcommand\labelitemi{--} 
  \item Configure a POOLNAME, "LIVE" is set per default.
  
  \item Configure the \texttt{cfl.connect} and \texttt{cfl.connect\_tc} commands which connect the Pool to the socket: 
  
  \item \texttt{cfl.connect(pool\_name, host, port, protocol)} 
  pool\_name = POOLNAME per default, host = "" local host, port XXXX needs to be configured according to the applications port.
\end{itemize}

\item For initial use, the code should be executed line by line using the upper left arrow. This approach makes it easier to follow the individual steps and to verify that each part of the process behaves as expected. Next to the line numbers, a little blue arrow indicates the current position in the script.

\item \texttt{cfl.start\_pmgr()} will open the Pool Manager.

\item After running \texttt{cfl.connect} and \texttt{cfl.connect\_tc} the connections should show up in the Pool Manager.

\item \texttt{\texttt{cfl.start\_pv()}} will open the Pool Viewer. If the application is sending continuous Housekeeping reports, these will already show up in the Pool Viewer.

\item \texttt{cfl.start\_monitor(POOLNAME)} will open the Parameter Monitor with which chosen sets of parameters can be monitored individually. 
\end{itemize}


In order to send a singular command to the application, the function \texttt{cfl.Tcsend\_DB()} may be used. This and other functions defined in section \ref{scripts} 
allow users to execute \textit{ccs} operations directly from the console of the editor or from within a Python script. For instance, the simple command (with the appropriate mib entry)

\begin{verbatim}
cfl.Tcsend_DB('AreYouAliveCmd', pool_name=POOLNAME)
\end{verbatim}

sends a TC(17,1) via the connected pool and returns the APID, Sequence count and internal time stamp of the sent TC as confirmation. 
...
A detailed overview of configuration files and user preferences is provided in Chapter~\ref{...}.
...
\begin{figure}[ht]
  ...
  \caption{Icons visible on top of the Pool Viewer}
...
Caution: This function is irreversibly and all data in the respective pool is lost!\\
...
\item Cal: Plots calibrated (engineering) values instead of raw data. The calibration is applied only to the next time step or to the next parameter added.

\item MIN / MAX: Define the visible range of the x-axis, allowing users to constrain the plotted interval.
...
It can be opened directly from the Pool Viewer via the \emph{Monitor} button (see Figure~\ref{fig:monitor_0}). \\
...
The reporting workflow is described in detail in Section~\ref{...}. \\
...
\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{test_editor.png}
...
\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{test_viewer.png}
...
The object cfl (short for \textit{ccs} Function Library) provides access to the internal command interface of the \textit{ccs} system. 
It serves as the main entry point for sending and receiving packets, handling telemetry, and interacting with the connected instruments.

Commonly used functions include, for example: \texttt{cfl.Tcsend\_DB()} – sends a TC packet defined in the MIB or \texttt{cfl.Tcbuild()} – builds a TC packet without sending it immediately. 
